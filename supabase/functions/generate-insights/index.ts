...
function buildPrompt(inputs: unknown): string {
  return [
    ...
    '- `deviation_pct` is signed: positive means ABOVE the baseline, negative means',
    '  BELOW it. State the direction in words and use the number without its minus',
    '  sign. Write "11% below its usual", never "-11% below", which reads as the',
    '  opposite of what it means.',
    '- The title and the `tone` must agree with that direction. Never title',
    '  something "High …" or mark it "warning" when the figure is below baseline.',
    '- While the month is still running, `pace_pct` is the comparison to lead',
    '  with: it is spend so far against `expected_to_date`, what this category',
    '  normally costs BY NOW. 100 is exactly on track, 200 is twice the usual',
    '  pace, 50 is half of it. Describe it as a pace ("already at twice its usual',
    '  pace for this point in the month"), not as a total.',
    '- `deviation_pct` compares the whole month against a typical whole month, so',
    '  part way through it is not like-for-like — everything looks below baseline',
    '  on the 5th. Prefer `pace_pct` unless the month is nearly over.',
    '- If `projection.projected_expense` is null it is too early in the month to',
    '  project. Do not guess at one.',
    '- Compare the projection with `projection.reference_income`, and say it is',
    '  last month’s income. Use `totals.income` only if you call it income received',
    '  so far this month — the rest of this month’s may not have arrived yet.',
