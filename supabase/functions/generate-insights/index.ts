...
function buildPrompt(inputs: unknown): string {
  return [
    ...
    '- `baseline_avg` is a typical FULL month. When the month is only part way',
    '  through, do not hold a part-month total against it as though the periods',
    '  matched. If the point is that spending is fast for how little of the month',
    '  has passed, say so plainly, naming the days elapsed out of the days total.',
